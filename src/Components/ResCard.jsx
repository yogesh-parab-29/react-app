...
const ResCard = ({ restaurant }) => {
  const { name, avgRating, sla, cuisines, areaName, cloudinaryImageId } = restaurant;
  return (
    <>
      <div className="w-56 h-64 ">
        <div className="w-full h-1/2 overflow-hidden rounded-xl shadow-md ">
          <img className="w-full h-full object-cover" src={RES_IMG_CDN_URL + cloudinaryImageId}></img>
        </div>
        ...
        <div className="truncate w-10/12 mx-auto text-sm">{cuisines.join(', ')}</div>
